"""
PDF Compiler implementation using Typst.
Compiles DocumentAST into publication-ready PDF books.
"""
...
logger = logging.getLogger("bookcraft.compiler.pdf")
...
def build_typst_document(ast: DocumentAST) -> str:
...
''')
    ...
    if ast.front_matter.copyright.enabled:
        ...
        lines.append(f'  Copyright © {c.year or 2026} {_escape_typst(c.holder or author)}\\')
...
class PdfCompiler(BaseCompiler):
    """PDF Compiler backed by Typst engine."""
    ...
    async def compile(
        ...
        typst_code = build_typst_document(ast)
        ...
        try:
            ...
            raise RuntimeError(f"Typst PDF compilation failed: {e}")

        download_url = self.get_download_url(pdf_filename)
        return pdf_path, download_url
